...
// raccourci pour mmap
#define MMAP(size)         ((char *)mmap(0, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0))
#define MUNMAP(addr, size) munmap(addr, size)

//   décrit un ensemble de pages
// La mémoire est découpée en zone de plusieurs pages. Tous les objets d'une zone contiennent des objets de la même taille.
// Chaque page d'une zone est hashée dans la table de hash heap_desc en utilisant les parties hautes/basses des pages
// qui la compose.
// une entrée contient soit
//   - 0 : ça veut dire que la page n'est pas mappée en mémoire
//   - un nombre positif : donne la taille des élements qui se trouvent dans cette zone
//   - un nombre négatif : donne le décalage (en page) par rapport au début de la zone
struct bank_desc {
	unsigned int pages[1<<LOW_BITS];
};
...
static struct bank_desc            heap_desc[1<<HIGH_BITS];
...
static inline unsigned int align(unsigned int n, unsigned int align) { 
	return (n + align - 1) & -align; 
}

// donne la taille réellement occupée d'un objet : taille de l'objet + taille de l'entête, aligné sur un OBJECT_ALIGN
static inline unsigned int total_size(int size) {
	// le 4 sert à chaîner les objets de taille 0 lorsqu'ils sont libres
	return align((size ? size : 4) + sizeof(struct object_header), OBJECT_ALIGN);
}
...
static inline void set_page_desc(char *page, int value) {
	// n est le numéro de la page
	int n = ((unsigned int)page)  >> OFFSET_BITS;
...
// hash une zone complete. La zone commence en zone, fait size octets, elle contient des elements de elmt_size
...
// alloue une nouvelle zone de taille size contenant des élements de taille elmt_size (taille complète avec header et aligné)
...
// réalise l'allocation de size octets. zero_filled indique si on doit la remplir avec des zéros.
static inline struct object_header *allocate_internal(unsigned int size, int zero_filled) {
	...
	if(allocator) {
		...
		if(allocator->first) {
			...
			// qui peut eventuellement être entièrement remplie
			if((allocator->current_zone += tot) > allocator->end_zone) {
				// dans ce cas, on alloue une nouvelle zone. On considère qu'il faut au moins 32 objets dans la zone pour le morcelement interne.
...
// réalise l'allocation et remplit de zero
...
struct object_header *toHeader(void *p) {
	...
	int n         = (unsigned int)p >> OFFSET_BITS;
	...
	if(elmt_size) {
		...
		unsigned int page_addr = n << OFFSET_BITS;

		// retrouve l'entête en suivant la règle suivante
		struct object_header *header = 
			(struct object_header *)(page_addr + elmt_size * (((unsigned int)p - page_addr) / elmt_size));
		...
		if((unsigned int)p - (unsigned int)toObject(header) < header->object_size)
			return header;
		else
			return 0;
	} else
